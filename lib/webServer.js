...
function createWebServer(tunnelServer, tftpServer, hostKeyPublic, hostKeyPrivate) {
  ...
  // One-time privileged node setup (service account, systemd unit, replication key) --
  // previously only reachable by running provisioning/ha-setup.sh over SSH, which this
  // appliance has no OS-level login for by default. Idempotent, safe to click again.
  app.post('/api/ha/setup', requireFullAuth, async (req, res) => {
    try {
      const result = await systemControl.setupHighAvailability();
      audit(req, 'ran HA node setup on this node (service account, systemd unit, replication key)');
  ...
  app.get('/api/ha/service-status', requireFullAuth, async (req, res) => {
    try {
      res.json(await systemControl.getHaServiceStatus());
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });

  app.post('/api/ha/start-service', requireFullAuth, async (req, res) => {
    try {
      await systemControl.startHaAgentService();
      audit(req, 'started the HA agent systemd service on this node');
      res.json({ ok: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  // Scan-then-trust, split into two steps so an admin confirms the fingerprint (shown in
  // the UI) against the peer itself before it's persisted -- the same trust decision an
  // interactive ssh session would ask for, just without a shell to ask it in.
  app.post('/api/ha/peer-hostkey/scan', requireFullAuth, async (req, res) => {
    const host = String((req.body && req.body.host) || '').trim();
    if (!host) return res.status(400).json({ error: 'host is required' });
    try {
      res.json(await systemControl.scanHaPeerHostKey(host));
    } catch (e) {
      res.status(502).json({ error: e.message });
    }
  });

  app.post('/api/ha/peer-hostkey/trust', requireFullAuth, async (req, res) => {
    const keyLine = String((req.body && req.body.keyLine) || '').trim();
    if (!keyLine) return res.status(400).json({ error: 'keyLine is required' });
    try {
      await systemControl.trustHaPeerHostKey(keyLine);
      audit(req, `trusted a peer SSH host key for HA replication (${keyLine.split(' ')[0]})`);
      res.json({ ok: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
