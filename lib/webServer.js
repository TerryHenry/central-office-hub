...
function createWebServer(tunnelServer, tftpServer, hostKeyPublic, hostKeyPrivate) {
  ...
  // Queues a fresh, per-site "sync-users" command for each site id. scope 'access' sends
  // only users with reach into the site, 'all' sends every hub user; an explicit scope is
  // remembered on the site, and later automatic re-syncs (user create/edit/delete) reuse it.
  // Unknown site ids are skipped. Returns {sites, users}: how many sites were queued and
  // the total accounts sent, so a caller can tell when a sync matched nobody.
  function queueUserSync(siteIds, scope) {
    let sites = 0;
    let users = 0;
    for (const siteId of new Set(siteIds)) {
      const site = configStore.findSiteById(siteId);
      if (!site) continue;
      if (scope) configStore.setSiteUserSyncScope(siteId, scope);
      const list = configStore.usersForSiteSync(siteId, scope || site.userSyncScope || 'access');
      configStore.queueCommand(siteId, 'sync-users', list);
      sites++;
      users += list.length;
    }
    return { sites, users };
  ...
  // Sites that were synced with every hub user -- any user change must reach them too.
  function allScopeSiteIds() {
    return configStore.listSites().filter((s) => s.userSyncScope === 'all').map((s) => s.id);
  }

  function normalizeScope(value) {
    return value === 'all' ? 'all' : 'access';
  }
  ...
  app.post('/api/sites/:id/sync-users', requireFullAuth, (req, res) => {
    const site = configStore.findSiteById(req.params.id);
    if (!site) return res.status(404).json({ error: 'site not found' });
    const { users } = queueUserSync([site.id], normalizeScope(req.body && req.body.scope));
    audit(req, `queued a user sync for site "${site.name}" (${users} account${users === 1 ? '' : 's'})`);
  ...
  app.post('/api/sites/sync-users/bulk', requireFullAuth, (req, res) => {
    ...
    const { sites: queued, users } = queueUserSync(siteIds, normalizeScope(req.body && req.body.scope));
    audit(req, `queued a user sync for ${queued} site${queued === 1 ? '' : 's'} (${users} account${users === 1 ? '' : 's'} in total)`);
    res.json({ ok: true, queued, users });
  });
  ...
  app.post('/api/users', requireFullAuth, (req, res) => {
    ...
    try {
      ...
      const pushed = queueUserSync([...pushTo, ...allScopeSiteIds()]).sites;
  ...
  app.post('/api/users/:id', requireFullAuth, (req, res) => {
    ...
    try {
      ...
      queueUserSync([...previousSites, ...(user.edgeSiteIds || []), ...allScopeSiteIds()]);
      res.json(userSummary(user));
    } catch (e) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.post('/api/users/:id/password', requireFullAuth, (req, res) => {
    ...
    try {
      ...
      queueUserSync([...(user.edgeSiteIds || []), ...allScopeSiteIds()]);
      res.json({ ok: true });
    } catch (e) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.delete('/api/users/:id', requireFullAuth, (req, res) => {
    ...
    if (user) queueUserSync([...(user.edgeSiteIds || []), ...allScopeSiteIds()]);
    res.json({ ok: true });
  });
