...
function createWebServer(tunnelServer, hostKeyPublic) {
  ...
  async function sitesWithStatus() {
    ...
    const hubAdminUsernames = configStore.listAdmins().map((a) => a.username).sort();
    return configStore.listSites().map((s) => ({
      ...
      // null = unknown (box hasn't reported admin usernames yet, e.g. an older version
      // or no heartbeat since this shipped) -- distinct from true/false, never guessed.
      adminsSynced: s.reportedAdminUsernames
        ? JSON.stringify(s.reportedAdminUsernames) === JSON.stringify(hubAdminUsernames)
        : null
    }));
  }
  ...
  app.post('/api/fleet/heartbeat', (req, res) => {
    ...
    res.json({
      ...
      payload: command && command.payload !== undefined ? command.payload : null
    });
  });
