...
const { validateForwardTarget } = require('./forwardTarget');
...
function createWebServer(tunnelServer, tftpServer, hostKeyPublic, hostKeyPrivate) {
  ...
  // ---------- Forwards (reach a device on a site's LAN through that site's tunnel) ----------
  const FORWARD_NAME_RE = /^[a-z0-9]([a-z0-9._-]{0,61}[a-z0-9])?$/;

  function forwardView(f) {
    const site = configStore.findSiteById(f.siteId);
    return { ...f, siteName: site ? site.name : null, siteConnected: !!site && siteRegistry.isConnected(site.id) };
  }

  app.get('/api/forwards', requireFullAuth, (req, res) => res.json(configStore.listForwards().map(forwardView)));

  app.post('/api/forwards', requireFullAuth, (req, res) => {
    const { id, siteId, notes } = req.body || {};
    const name = String((req.body && req.body.name) || '').trim().toLowerCase();
    const host = String((req.body && req.body.host) || '').trim();
    const port = Number(req.body && req.body.port);
    if (!FORWARD_NAME_RE.test(name)) {
      return res.status(400).json({ error: 'the name must be 1-63 letters, digits, dots, dashes or underscores (it is what you type in the SSH command)' });
    }
    const invalid = validateForwardTarget(host, port);
    if (invalid) return res.status(400).json({ error: invalid });
    try {
      const forward = configStore.saveForward({ id, name, siteId, host, port, notes: String(notes || '').slice(0, 200) });
      audit(req, `${id ? 'updated' : 'created'} forward "${forward.name}"`);
      res.json(forwardView(forward));
    } catch (e) {
      res.status(400).json({ error: e.message });
    }
  });

  app.delete('/api/forwards/:id', requireFullAuth, (req, res) => {
    const forward = configStore.findForwardById(req.params.id);
    configStore.deleteForward(req.params.id);
    audit(req, `deleted forward "${forward ? forward.name : req.params.id}"`);
    res.json({ ok: true });
  });

  // Asks the site to actually connect to the device (and hang up), so a typo or a firewalled
  // device shows up here rather than on someone's first SSH attempt.
  app.post('/api/forwards/:id/test', requireFullAuth, async (req, res) => {
    const forward = configStore.findForwardById(req.params.id);
    if (!forward) return res.status(404).json({ error: 'forward not found' });
    if (!siteRegistry.isConnected(forward.siteId)) return res.status(400).json({ error: 'that site is not currently connected' });
    try {
      await siteRegistry.probeForward(forward.siteId, forward.host, forward.port);
      res.json({ ok: true });
    } catch (e) {
      res.status(502).json({ error: e.message });
    }
