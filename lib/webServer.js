...
const siteShell = require('./siteShell');
...
const alertWebhook = require('./alertWebhook');
const { BatchManager, parseScript } = require('./batchRunner');
...
function createWebServer(tunnelServer, tftpServer, hostKeyPublic, hostKeyPrivate) {
  ...
  function allSessions() {
    return [...tunnelServer.listSessions(), ...webConsole.listSessions(), ...siteShell.listSessions()];
  }

  tunnelServer.on('log', onLog);
  tunnelServer.on('sessions-changed', () => broadcast('sessions', allSessions()));
  tunnelServer.on('site-connected', (site) => fireAlert('site_online', { siteId: site.id, siteName: site.name }));
  tunnelServer.on('site-disconnected', (site) => fireAlert('site_offline', { siteId: site.id, siteName: site.name }));
  selfUpdate.on('log', onLog);
  webConsole.on('log', onLog);
  webConsole.on('sessions-changed', () => broadcast('sessions', allSessions()));
  siteShell.on('log', onLog);
  siteShell.on('sessions-changed', () => broadcast('sessions', allSessions()));
  ...
  function audit(req, message) {
    ...
    fireAlert('audit', { user: who, message });
  ...
  // Checks the relevant notifyOn* toggle itself, so every call site just names the
  // event instead of re-checking config -- not awaited by any caller (fire-and-forget,
  // same as audit()'s own syslog send above), but does log a delivery failure so it's
  // visible somewhere instead of silently vanishing.
  const ALERT_EVENTS = {
    site_offline: 'notifyOnSiteOffline',
    site_online: 'notifyOnSiteOnline',
    admin_lockout: 'notifyOnLockout',
    admin_login: 'notifyOnAdminLogin',
    admin_login_failed: 'notifyOnLoginFailure',
    site_enrolled: 'notifyOnEnrollment',
    audit: 'notifyOnAudit',
    ups_on_battery: 'notifyOnUpsPower',
    ups_online: 'notifyOnUpsPower',
    ups_low_battery: 'notifyOnUpsPower'
  };
  function fireAlert(event, data) {
    const alerts = configStore.getConfig().alerts;
    if (!alerts.webhookUrl) return;
    const toggle = ALERT_EVENTS[event];
    if (toggle && !alerts[toggle]) return;
    alertWebhook.send(alerts.webhookUrl, event, data).then((result) => {
      if (!result.ok) onLog(`[${logTimestamp()}] [ALERT] webhook delivery failed: ${result.error}`);
    });
  }

  // Heartbeat reports UPS state on every cycle (30s), not just when it changes -- this
  // is what turns that into actual transition events, the same distinction
  // site-connected/site-disconnected already draw for tunnel state instead of firing on
  // every heartbeat a site happens to still be online. previous/current are each either
  // null (not configured / not reporting) or upsMonitor.summarize()'s shape from the
  // appliance side ({onBattery, lowBattery, ...}).
  function notifyUpsTransitions(site, previous, current) {
    const wasOnBattery = !!(previous && previous.onBattery);
    const isOnBattery = !!(current && current.onBattery);
    const wasLowBattery = !!(previous && previous.lowBattery);
    const isLowBattery = !!(current && current.lowBattery);
    if (isOnBattery && !wasOnBattery) {
      fireAlert('ups_on_battery', { siteId: site.id, siteName: site.name, batteryCharge: current.batteryCharge });
    } else if (!isOnBattery && wasOnBattery) {
      fireAlert('ups_online', { siteId: site.id, siteName: site.name });
    }
    if (isLowBattery && !wasLowBattery) {
      fireAlert('ups_low_battery', { siteId: site.id, siteName: site.name, batteryCharge: current.batteryCharge });
    }
  }
  ...
  // ---------- Admin idle timeout ----------
  // Only deliberate activity keeps an admin session alive: any non-GET request counts (the
  // UI sends a throttled POST /api/session/touch on real mouse/keyboard use), while
  // background polling GETs do not -- otherwise the dashboard's own auto-refresh would
  // keep an unattended session open forever.
  function adminSessionExpired(req) {
    if (!(req.session && req.session.authenticated)) return false;
    const minutes = Number(configStore.getConfig().web.idleTimeoutMinutes);
    const limit = minutes > 0 ? minutes * 60 * 1000 : 0;
    const now = Date.now();
    if (limit && req.session.lastActivity && now - req.session.lastActivity > limit) return true;
    if (!req.session.lastActivity || req.method !== 'GET') req.session.lastActivity = now;
    return false;
  }

  // Drops only the admin login, keeping the session itself (and its CSRF token) so the
  // login form that follows can still submit.
  function clearAdminAuth(req) {
    const who = req.session.username;
    delete req.session.authenticated;
    delete req.session.adminId;
    delete req.session.username;
    delete req.session.lastActivity;
    if (who) onLog(`[${logTimestamp()}] [AUDIT] "${who}" was logged out after being idle`);
  }
  function requireAuth(req, res, next) {
    if (req.session && req.session.authenticated) {
      if (adminSessionExpired(req)) {
        clearAdminAuth(req);
  ...
  app.get('/api/session', (req, res) => {
    if (adminSessionExpired(req)) clearAdminAuth(req);
    ...
    res.json({
      ...
      idleTimeoutMinutes: configStore.getConfig().web.idleTimeoutMinutes,
  ...
  app.post('/api/login', (req, res) => {
    ...
    if (admin && configStore.verifyPassword(password, admin.passwordSalt, admin.passwordHash)) {
      ...
      loginSession(
        ...
        (csrfToken) => {
          ...
          fireAlert('admin_login', { username: admin.username, ip });
          res.json({
            ok: true,
    ...
    fireAlert('admin_login_failed', { username, ip });
    // isLocked can only just now have become true -- a request that arrives already
    // locked returns 429 above, before ever reaching recordFailure, so this fires
    // exactly once per lockout (the attempt that tips it over), not on every
    // subsequent attempt against an already-locked pair.
    if (loginThrottle.isLocked(ip, username)) {
      fireAlert('admin_lockout', { username, ip });
    }
  ...
  app.post('/api/session/touch', requireAuth, (req, res) => res.json({ ok: true }));

  app.get('/api/session-settings', requireFullAuth, (req, res) => {
    res.json({ idleTimeoutMinutes: configStore.getConfig().web.idleTimeoutMinutes });
  });

  app.post('/api/session-settings', requireFullAuth, (req, res) => {
    const minutes = Number(req.body && req.body.idleTimeoutMinutes);
    if (!Number.isInteger(minutes) || minutes < 0 || minutes > 1440) {
      return res.status(400).json({ error: 'the idle timeout must be a whole number of minutes from 0 (never) to 1440' });
    }
    configStore.updateWeb({ idleTimeoutMinutes: minutes });
    audit(req, `set the admin idle timeout to ${minutes ? `${minutes} minute${minutes === 1 ? '' : 's'}` : 'never'}`);
  ...
  app.post('/api/sites/:id/detect-baud', requireFullAuth, async (req, res) => {
    const site = configStore.findSiteById(req.params.id);
    if (!site) return res.status(404).json({ error: 'site not found' });
    if (!siteRegistry.isConnected(site.id)) {
      return res.status(400).json({ error: 'that site is not currently connected' });
    ...
    try {
      res.json(await siteRegistry.requestBaudDetect(site.id, String((req.body && req.body.path) || '')));
    } catch (e) {
      res.status(502).json({ error: e.message });
    }
  });

  app.get('/api/sites/:id/lldp', requireFullAuth, async (req, res) => {
    const site = configStore.findSiteById(req.params.id);
    if (!site) return res.status(404).json({ error: 'site not found' });
    if (!siteRegistry.isConnected(site.id)) {
      return res.status(400).json({ error: 'that site is not currently connected' });
    ...
    try {
      res.json(await siteRegistry.requestLldp(site.id));
    } catch (e) {
      res.status(502).json({ error: e.message });
    }
  });

  app.post('/api/sites/:id/lldp', requireFullAuth, (req, res) => {
    const site = configStore.findSiteById(req.params.id);
    if (!site) return res.status(404).json({ error: 'site not found' });
    const { enabled, cdp, fdp } = req.body || {};
    configStore.queueCommand(site.id, 'set-lldp', { enabled: !!enabled, cdp: !!cdp, fdp: !!fdp });
    audit(req, `queued neighbor-discovery settings for site "${site.name}" (LLDP ${enabled ? 'on' : 'off'}, CDP ${cdp ? 'on' : 'off'}, FDP ${fdp ? 'on' : 'off'})`);
    res.json({ ok: true });
  });

  // Read-only, unlike lldp's POST above -- deliberately no way to configure a site's UPS
  // driver/port from the hub, same reasoning as remote shell: physical hardware wiring
  // is that site's own admin's call to make locally, not something pushed from here.
  app.get('/api/sites/:id/ups', requireFullAuth, async (req, res) => {
    const site = configStore.findSiteById(req.params.id);
    if (!site) return res.status(404).json({ error: 'site not found' });
    if (!siteRegistry.isConnected(site.id)) {
      return res.status(400).json({ error: 'that site is not currently connected' });
    ...
    try {
      res.json(await siteRegistry.requestUpsStatus(site.id));
    } catch (e) {
      res.status(502).json({ error: e.message });
    }
  });
  ...
  // way. Spooled to a temp file on disk (not held in memory) and streamed to each site
  // with backpressure, so a large image can't exhaust RAM and take every tunnel down.
  const siteUploadTmpDir = path.join(os.tmpdir(), 'central-office-uploads');
  fs.mkdirSync(siteUploadTmpDir, { recursive: true });
  const siteTftpUpload = multer({
    storage: multer.diskStorage({ destination: siteUploadTmpDir }),
    limits: { fileSize: 1024 * 1024 * 1024 }
  });
  const removeTmpUpload = (file) => {
  ...
  app.post('/api/sites/:id/tftp-upload', requireFullAuth, (req, res) => {
    siteTftpUpload.single('file')(req, res, async (err) => {
      try {
        ...
        try {
          const result = await siteRegistry.uploadTftpFile(site.id, filename, req.file.path, req.file.size);
          audit(req, `uploaded "${filename}" (${req.file.size} bytes) to site "${site.name}"'s TFTP directory`);
          res.json({ ok: true, filename: result.filename, bytesWritten: result.bytesWritten });
        } catch (e) {
          res.status(502).json({ error: e.message });
        }
      } finally {
        removeTmpUpload(req.file);
      }
    });
  });

  // Bulk: push the same file to several sites at once, in parallel -- each site succeeds
  // or fails independently (one being offline shouldn't block the others). Only a few
  // transfers run at once so a big fan-out doesn't saturate the hub's uplink/CPU and
  // starve the other tunnels' traffic.
  ...
  app.post('/api/sites/tftp-upload/bulk', requireFullAuth, (req, res) => {
    siteTftpUpload.single('file')(req, res, async (err) => {
      try {
        ...
        const results = new Array(siteIds.length);
        let next = 0;
        const worker = async () => {
          while (next < siteIds.length) {
            ...
            if (!site) {
              results[i] = { siteId, ok: false, error: 'site not found' };
            } else if (!siteRegistry.isConnected(siteId)) {
              results[i] = { siteId, name: site.name, ok: false, error: 'not currently connected' };
            } else {
              try {
                await siteRegistry.uploadTftpFile(siteId, filename, req.file.path, req.file.size);
                results[i] = { siteId, name: site.name, ok: true };
              } catch (e) {
                results[i] = { siteId, name: site.name, ok: false, error: e.message };
              }
            }
          }
        };
        await Promise.all(Array.from({ length: Math.min(BULK_UPLOAD_CONCURRENCY, siteIds.length) }, worker));
  ...
  app.post('/api/enrollment-tokens', requireFullAuth, (req, res) => {
    const { name, expiresInMinutes, siteId } = req.body || {};
    if (siteId) {
      const site = configStore.findSiteById(siteId);
      if (!site) return res.status(404).json({ error: 'site not found' });
      const token = configStore.createEnrollmentToken(site.name, Number(expiresInMinutes) || 60, site.id);
      audit(req, `issued a re-enrollment token for site "${site.name}"`);
  ...
  app.post('/api/fleet/enroll', (req, res) => {
    ...
    try {
      const { site, reenrolled } = configStore.redeemEnrollmentToken(token, publicKey);
      loginThrottle.recordSuccess(ip, 'fleet-enroll');
      // A re-keyed site's old box must not stay connected on the key that no longer matches.
      if (reenrolled) siteRegistry.unregister(site.id);
      onLog(`[${logTimestamp()}] [AUDIT] site "${site.name}" ${reenrolled ? 're-enrolled a replacement box' : 'self-enrolled'} via token`);
      fireAlert('site_enrolled', { siteId: site.id, siteName: site.name });
  ...
  app.post('/api/fleet/heartbeat', (req, res) => {
    ...
    // Captured before recordHeartbeat overwrites it -- comparing against the freshly
    // reported data.ups below is what turns "on battery" from a fact repeated on every
    // 30s heartbeat into an actual transition to alert on, the same distinction
    // site-connected/site-disconnected already make for tunnel state.
    const previousUps = site.ups;
    configStore.recordHeartbeat(site.id, {
      version: data.version,
      ports: data.ports,
      sshEnabled: data.sshEnabled,
      webTerminalEnabled: data.webTerminalEnabled,
      remoteShellEnabled: data.remoteShellEnabled,
      ups: data.ups,
      adminsFingerprint: data.adminsFingerprint,
      tftp: data.tftp,
      hubKeyPinned: data.hubKeyPinned
    });
    notifyUpsTransitions(site, previousUps, data.ups);
  ...
  function userSummary(u) {
    return {
      ...
      username: u.username,
      groupIds: u.groupIds,
      captureEnabled: !!u.captureEnabled,
      edgeSiteIds: u.edgeSiteIds || [],
      edgePermission: u.edgePermission || 'read-write',
      createdAt: u.createdAt
    };
  }

  // Queues a fresh, per-site "sync-users" command for each site id (each site gets only
  // the accounts it should carry -- see configStore.usersForSiteSync). Unknown site ids
  // are skipped. Returns how many sites were queued.
  function queueUserSync(siteIds) {
    let queued = 0;
    for (const siteId of new Set(siteIds)) {
      if (!configStore.findSiteById(siteId)) continue;
      configStore.queueCommand(siteId, 'sync-users', configStore.usersForSiteSync(siteId));
      queued++;
    }
    return queued;
  }

  function validEdgeSiteIds(value) {
    return Array.isArray(value) ? value.filter((id) => typeof id === 'string' && configStore.findSiteById(id)) : [];
  }

  app.post('/api/sites/:id/sync-users', requireFullAuth, (req, res) => {
    const site = configStore.findSiteById(req.params.id);
    if (!site) return res.status(404).json({ error: 'site not found' });
    queueUserSync([site.id]);
    audit(req, `queued a user sync for site "${site.name}"`);
    res.json({ ok: true });
  });

  app.post('/api/sites/sync-users/bulk', requireFullAuth, (req, res) => {
    const { siteIds } = req.body || {};
    if (!Array.isArray(siteIds) || siteIds.length === 0) {
      return res.status(400).json({ error: 'siteIds must be a non-empty array' });
    }
    const queued = queueUserSync(siteIds);
    audit(req, `queued a user sync for ${queued} site${queued === 1 ? '' : 's'}`);
    res.json({ ok: true, queued });
  });
  ...
  app.post('/api/users', requireFullAuth, (req, res) => {
    const { username, password, groupIds, captureEnabled, edgeSiteIds, edgePermission } = req.body || {};
    if (!username || !username.trim()) return res.status(400).json({ error: 'a username is required' });
    const pwError = checkPassword(password);
    if (pwError) return res.status(400).json({ error: pwError });
    try {
      const pushTo = validEdgeSiteIds(edgeSiteIds);
      const user = configStore.createUser(username.trim(), password, groupIds, captureEnabled, pushTo, edgePermission);
      audit(req, `created console user "${user.username}"`);
      const pushed = queueUserSync(pushTo);
      if (pushed) audit(req, `queued console user "${user.username}" for push to ${pushed} site${pushed === 1 ? '' : 's'}`);
  ...
  app.post('/api/users/:id', requireFullAuth, (req, res) => {
    const { username, groupIds, captureEnabled, edgeSiteIds, edgePermission } = req.body || {};
    try {
      const before = configStore.findUserById(req.params.id);
      const previousSites = before && Array.isArray(before.edgeSiteIds) ? [...before.edgeSiteIds] : [];
      const user = configStore.updateUser(req.params.id, {
        username,
        groupIds,
        captureEnabled,
        edgeSiteIds: Array.isArray(edgeSiteIds) ? validEdgeSiteIds(edgeSiteIds) : undefined,
        edgePermission
      });
      audit(req, `updated console user "${user.username}"`);
      // Re-sync every explicitly-pushed site the user was on or is now on, so both
      // additions and removals of the local login take effect.
      queueUserSync([...previousSites, ...(user.edgeSiteIds || [])]);
      res.json(userSummary(user));
    } catch (e) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.post('/api/users/:id/password', requireFullAuth, (req, res) => {
    ...
    try {
      ...
      queueUserSync(user.edgeSiteIds || []);
      res.json({ ok: true });
    } catch (e) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.delete('/api/users/:id', requireFullAuth, (req, res) => {
    ...
    if (user) queueUserSync(user.edgeSiteIds || []);
    res.json({ ok: true });
  });
  ...
  app.post('/api/terminal/logout', (req, res) => {
    // Destroys the whole session: clearing only the console-user field left an admin
    // (who reaches this page through their admin session) still signed in, so "Sign Out"
    // appeared to do nothing -- the reload just landed them back on the picker.
    const who = req.session && ((req.session.consoleUser && req.session.consoleUser.username) || req.session.username);
    req.session.destroy(() => {
  ...
  // Admin-only, unlike /terminal above -- no console-user login screen, since this
  // capability was never offered to console users in the first place. Auth is enforced
  // client-side (shell.js checks /api/session) and again at the WebSocket upgrade
  // (siteShell.js), the same two-layer pattern /terminal itself relies on.
  app.get('/shell', (req, res) => {
    res.sendFile(path.join(__dirname, '..', 'webui', 'shell.html'));
  });

  // ---------- Batch actions (run a script against many ports, with a per-device record) ----------
  const batchManager = new BatchManager({
    dataDir: configStore.DATA_DIR,
    log: (line) => onLog(`[${logTimestamp()}] [BATCH] ${line}`),
    describe: (siteId, portId) => {
      const site = configStore.findSiteById(siteId);
      const port = site && site.ports.find((p) => p.id === portId);
      return port ? { siteName: site.name, portLabel: port.label } : null;
    },
    // The same tunnelled connection a web console uses to reach the port.
    openPort: (siteId, portId) => siteRegistry.openPort(siteId, portId)
  });

  app.get('/api/batch-templates', requireFullAuth, (req, res) => res.json(configStore.listBatchTemplates()));

  app.post('/api/batch-templates', requireFullAuth, (req, res) => {
    const { id, name, script } = req.body || {};
    if (!name || !String(name).trim()) return res.status(400).json({ error: 'a template name is required' });
    try {
      parseScript(script);
    } catch (e) {
      return res.status(400).json({ error: e.message });
    }
    const template = configStore.saveBatchTemplate({ id, name: String(name).trim().slice(0, 80), script });
    audit(req, `saved batch template "${template.name}"`);
    res.json(template);
  });

  app.delete('/api/batch-templates/:id', requireFullAuth, (req, res) => {
    const template = configStore.listBatchTemplates().find((t) => t.id === req.params.id);
    configStore.deleteBatchTemplate(req.params.id);
    audit(req, `deleted batch template "${template ? template.name : req.params.id}"`);
    res.json({ ok: true });
  });

  app.get('/api/batches', requireFullAuth, (req, res) => res.json(batchManager.list()));

  app.get('/api/batches/:id', requireFullAuth, (req, res) => {
    const run = batchManager.get(req.params.id);
    if (!run) return res.status(404).json({ error: 'batch not found' });
    res.json(run);
  });

  app.post('/api/batches', requireFullAuth, (req, res) => {
    const { name, script, targets, username, password } = req.body || {};
    try {
      const run = batchManager.start({
        name,
        script,
        targets,
        vars: { username, password },
        startedBy: (req.session && req.session.username) || null
      });
      // The script itself is not logged here (it can name sensitive commands) -- only its size.
      audit(req, `started batch "${run.name}" on ${run.targetCount} port${run.targetCount === 1 ? '' : 's'}`);
      res.json(run);
    } catch (e) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.post('/api/batches/:id/cancel', requireFullAuth, (req, res) => {
    try {
      batchManager.cancel(req.params.id);
      audit(req, 'cancelled a running batch');
      res.json({ ok: true });
    } catch (e) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.delete('/api/batches/:id', requireFullAuth, (req, res) => {
    try {
      batchManager.remove(req.params.id);
      res.json({ ok: true });
    } catch (e) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.post('/api/sessions/:id/kick', requireFullAuth, (req, res) => {
    ...
    siteShell.kickSession(req.params.id);
  ...
  // ---------- Alert webhooks ----------
  app.get('/api/alerts', requireFullAuth, (req, res) => res.json(configStore.getConfig().alerts));

  app.post('/api/alerts', requireFullAuth, (req, res) => {
    const body = req.body || {};
    ...
    const result = configStore.updateAlerts({
      webhookUrl: trimmedUrl,
      ...Object.fromEntries(Object.values(ALERT_EVENTS).map((key) => [key, !!body[key]]))
    });
    audit(req, `${trimmedUrl ? 'updated' : 'cleared'} the alert webhook`);
    res.json(result);
  });

  app.post('/api/alerts/test', requireFullAuth, async (req, res) => {
    const alerts = configStore.getConfig().alerts;
    if (!alerts.webhookUrl) return res.status(400).json({ error: 'set a webhook URL first' });
    const result = await alertWebhook.send(alerts.webhookUrl, 'test', { message: `Test alert from ${os.hostname()}` });
    if (!result.ok) return res.status(502).json({ error: result.error });
    res.json({ ok: true });
  });

  // ---------- Log / live events ----------
  // How many entries the Log tab / server.log keep (oldest are dropped past this).
  app.get('/api/log-settings', requireFullAuth, (req, res) => {
    res.json({ maxEntries: configStore.getConfig().log.maxEntries });
  });

  app.post('/api/log-settings', requireFullAuth, (req, res) => {
    const maxEntries = Number(req.body && req.body.maxEntries);
    if (!Number.isInteger(maxEntries) || maxEntries < 100 || maxEntries > 100000) {
      return res.status(400).json({ error: 'the maximum must be a whole number from 100 to 100000' });
    }
    configStore.updateLog({ maxEntries });
    logStore.setMaxLines(maxEntries);
    audit(req, `set the maximum log entries to ${maxEntries}`);
    res.json({ maxEntries });
  });
  ...
  // ---------- LLDP / CDP / FDP neighbor discovery (lldpd, via the privileged helper) ----------
  app.get('/api/lldp', requireFullAuth, async (req, res) => {
    const status = await lldp.getStatus();
    let neighbors = [];
    ...
    res.json({ ...status, neighbors, neighborsError });
  });

  app.post('/api/lldp', requireFullAuth, async (req, res) => {
    const { enabled, cdp, fdp } = req.body || {};
    try {
      const status = await lldp.setConfig({ enabled: !!enabled, cdp: !!cdp, fdp: !!fdp });
      audit(req, `${enabled ? 'enabled' : 'disabled'} LLDP neighbor discovery${enabled ? ` (CDP ${cdp ? 'on' : 'off'}, FDP ${fdp ? 'on' : 'off'})` : ''}`);
      res.json(status);
    } catch (e) {
      res.status(400).json({ error: e.message });
    }
  ...
  function attachTerminalSocket(httpsServer) {
    ...
    siteShell.attach(httpsServer, sessionMiddleware);
  }
