...
function createWebServer(tunnelServer, tftpServer, hostKeyPublic, hostKeyPrivate) {
  ...
  function fireAlert(event, data) {
    ...
    if (event === 'site_offline' && !alerts.notifyOnSiteOffline) return;
    if (event === 'admin_lockout' && !alerts.notifyOnLockout) return;
  ...
  app.post('/api/alerts', requireFullAuth, (req, res) => {
    const { webhookUrl, notifyOnSiteOffline, notifyOnLockout } = req.body || {};
    ...
    const result = configStore.updateAlerts({
      ...
      notifyOnSiteOffline: !!notifyOnSiteOffline,
