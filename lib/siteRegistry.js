...
class SiteRegistry extends EventEmitter {
  ...
  /** Asks a connected site to open a TCP connection to host:port on its own network (see the
   * edge's openForwardSession). Resolves {sock, leftover} once the site answers "ok" -- sock is
   * then a raw bridge to the device, leftover any bytes that arrived with the answer -- and
   * rejects with the site's reason (refused, unreachable, forwarding disabled...). */
  openForward(siteId, host, port, probe = false, timeoutMs = 12000) {
    return this.openTunneledConnection(siteId, { forward: true, host, port, probe }).then(
      (sock) =>
        new Promise((resolve, reject) => {
          ...
          const onData = (chunk) => {
            ...
            if (nl === -1) {
              if (buf.length > 512) fail(new Error('the site sent an unexpected reply'));
              return;
            }
            const line = buf.slice(0, nl).toString('utf8').trim();
            const leftover = buf.slice(nl + 1);
            if (line !== 'ok') return fail(new Error(line.replace(/^err\s*/, '') || 'the site refused the connection'));
            settled = true;
            clearTimeout(timer);
          ...
          const onClose = () => fail(new Error('the site closed the connection before answering'));
          timer = setTimeout(() => fail(new Error('timed out waiting for the site to reach that device')), timeoutMs);
          sock.on('data', onData);
  ...
  /** Reachability check only: connects, then drops the connection immediately. */
  probeForward(siteId, host, port) {
    return this.openForward(siteId, host, port, true).then(({ sock }) => {
      sock.destroy();
      return true;
    });
  }
