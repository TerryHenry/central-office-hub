...
const SIGNATURE_NAME = 'central-office-app.tar.gz.sig';
...
// itself. For an "install on an existing host" deployment this arrives for free as
// part of the git clone (it's a plain file at the repo root); for the VM appliance
// (build-vm.sh) it's written directly into place by the cloud-init user-data script,
// outside the tarball's own extraction. Either way, a trust anchor that a future
// update package could rewrite isn't a trust anchor: a single legitimately-signed-but
// -malicious release could otherwise swap in an attacker-controlled key and have every
// later release verify against that instead. A fresh clone/deploy, or a deliberate
// manual change, is the only way this ever changes. Separate keypair from the
// appliance's own (SerialKillerTermServer) -- different repos, different release
// artifacts, so a compromise of one doesn't hand over the other.
...
// Adding an item here only takes full effect starting with the update AFTER the one
// that ships this change -- applyUpdate() below is executed by the CURRENTLY RUNNING
// (old) code, so a box updating from a pre-change version to the one that adds a new
// item still uses the old SWAP_ITEMS to decide what to move out of STAGING_DIR, even
// though the new item is sitting right there in the download. It arrives correctly
// starting with that box's *next* update, once it's actually running this new list.
// Found the hard way with HANDBOOK.html/QUICKSTART.html: the fix landed, boxes updated
// to the release that shipped it, and the docs were still missing -- a second update
// (to nothing more than a version bump) was what actually delivered them.
...
class SelfUpdateManager extends EventEmitter {
  ...
  async checkForUpdate() {
    ...
    return {
      ...
      // still reported as "an update exists," just not one this hub can self-apply. An
      // unsigned release counts the same way: the checksum alone only proves the
  ...
  async applyUpdate() {
    ...
    try {
      ...
      // key that ships with the hub itself, not fetched from GitHub, actually proves
      // authorship. Signs the checksum file's own bytes (see scripts/sign-release.js),
      // which transitively covers the tarball too, since a tampered tarball would
      // already have failed the checksum comparison above.
