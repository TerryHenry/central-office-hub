...
// Cap on simultaneous forwarded connections per admin SSH connection.
const MAX_FORWARD_CHANNELS = 100;
...
class TunnelServerManager extends EventEmitter {
  ...
  handleConnection(client) {
    ...
    let authedSite = null;
    let authedUser = null;
    let authedEnrollToken = null;
    let pendingTotpAdmin = null; // password accepted, second factor (keyboard-interactive) still owed
    const remoteIp = sock ? sock.remoteAddress : 'unknown';

    client.on('authentication', (ctx) => {
      if (ctx.username === '_enroll') {
        // A not-yet-enrolled box, proving only that it holds a live enrollment token --
        // there's no site key to check yet, so the token itself (normally spent over
        // HTTPS via POST /api/fleet/enroll) doubles as this connection's password.
        // findUsableEnrollmentToken is a read-only check (same rule POST /api/fleet/enroll
        // already enforces) -- it doesn't consume the token; only the actual enroll call
        // this connection goes on to make does that, via configStore.redeemEnrollmentToken.
        if (ctx.method !== 'password') return ctx.reject(['password']);
        if (!configStore.findUsableEnrollmentToken(ctx.password)) return ctx.reject(['password']);
        authedEnrollToken = ctx.password;
        return ctx.accept();
      }
      ...
      if (ctx.method === 'keyboard-interactive') {
        // Only reachable after an admin's password already partially succeeded -- a client
        // can't jump straight to this method.
        if (!pendingTotpAdmin || pendingTotpAdmin.username !== ctx.username) return ctx.reject(['password']);
        const totpAdmin = pendingTotpAdmin;
        const throttleKey = `ssh-2fa:${totpAdmin.username}`;
        if (loginThrottle.isLocked(remoteIp, throttleKey)) {
          this.log(`2FA locked out for "${totpAdmin.username}" from ${remoteInfo}`);
          return ctx.reject([]);
        }
        ctx.prompt([{ prompt: 'Verification code: ', echo: false }], 'Two-Factor Authentication', '', (responses) => {
          const token = (responses && responses[0]) || '';
          if (totp.verifyToken(totpAdmin.totpSecret, token)) {
            loginThrottle.recordSuccess(remoteIp, throttleKey);
            pendingTotpAdmin = null;
            authedAdmin = totpAdmin;
            return ctx.accept();
          }
          loginThrottle.recordFailure(remoteIp, throttleKey);
          this.log(`Wrong 2FA code for "${totpAdmin.username}" from ${remoteInfo}`);
      ...
      if (admin && configStore.verifyPassword(ctx.password, admin.passwordSalt, admin.passwordHash)) {
        // An admin with 2FA on gets the same second step here as in the web UI -- SSH access
        // (a shell, and now forwards into site networks) shouldn't bypass it.
        if (admin.totpEnabled) {
          pendingTotpAdmin = admin;
          return ctx.reject(['keyboard-interactive'], true);
        }
        authedAdmin = admin;
        return ctx.accept();
    ...
    client.on('ready', () => {
      ...
      if (authedEnrollToken) {
        this.log(`Enrollment connection authenticated from ${remoteInfo}`);
        this.handleEnrollConnection(client);
        return;
      }
      if (authedAdmin) this.attachAdminForwarding(client, authedAdmin, remoteInfo);
    ...
    client.on('close', () => {
      if (authedSite) {
        ...
        // Listened for by webServer.js to fire the site-offline webhook alert, if one's
        // configured -- kept as a plain event here rather than importing the alerting
        // module directly, so this networking-layer file stays free of any awareness of
        // HTTP delivery, retries, or webhook config.
        this.emit('site-disconnected', authedSite);
      } else if (authedAdmin) {
      ...
      } else if (authedEnrollToken) {
        this.log(`Enrollment connection closed (${remoteInfo})`);
      }
    });
    client.on('error', (err) => this.log(`Client error (${remoteInfo}): ${err.message}`));
  }

  /** Lets an authenticated ADMIN reach a named Forward with an ordinary SSH port forward:
   *    ssh -N -L 8443:sw1:443 -p 443 admin@hub
   * The destination "host" of that request is the forward's NAME, not a network address -- the
   * hub looks it up, checks the port matches the forward, and asks the forward's site to open
   * the real connection on its own LAN. Nothing else is reachable this way, and console users
   * are never offered it. Every open and every refusal is written to the log. */
  attachAdminForwarding(client, admin, remoteInfo) {
    let open = 0;
    client.on('tcpip', (accept, reject, info) => {
      const wanted = `${info.destIP}:${info.destPort}`;
      const deny = (why) => {
        this.log(`[AUDIT] "${admin.username}" forward to ${wanted} refused: ${why}`);
        reject();
      };
      const fwd = configStore.findForwardByName(info.destIP);
      if (!fwd) return deny('no forward by that name (define it on the Forwards tab)');
      if (info.destPort !== fwd.port) return deny(`forward "${fwd.name}" uses port ${fwd.port}`);
      if (open >= MAX_FORWARD_CHANNELS) return deny('too many open forwarded connections');
      const site = configStore.findSiteById(fwd.siteId);
      if (!site) return deny('its site no longer exists');
      open += 1;
      let released = false;
      const release = () => {
        if (released) return;
      ...
      siteRegistry
        .openForward(site.id, fwd.host, fwd.port)
        .then(({ sock, leftover }) => {
          let channel;
          try {
            channel = accept();
          ...
          this.log(`[AUDIT] "${admin.username}" opened forward "${fwd.name}" -> ${site.name} ${fwd.host}:${fwd.port} (from ${remoteInfo})`);
  ...
  /** Bridges every direct-tcpip channel request on this connection straight to the hub's
   * own internal admin API listener, ignoring whatever destination the client actually
   * asked for -- forwardOut's own signature requires *some* host/port, but this side never
   * treats it as a general-purpose proxy. Shared by an already-enrolled site's persistent
   * tunnel (openHubApiChannel, for tunneled heartbeat/backup) and a not-yet-enrolled box's
   * one-shot enrollment connection below -- both only ever reach this same internal API,
   * exactly as either could already do with a direct connection to the hub's public web
   * port; only the network path changes, not what's reachable. */
  attachInternalApiBridge(client) {
  ...
  /** A not-yet-enrolled box's one-shot connection (see the '_enroll' branch in
   * handleConnection's 'authentication' handler): it only ever needs to run its existing
   * POST /api/fleet/enroll call, so this just opens the bridge and nothing else -- no
   * tcpip-forward handling, since this connection isn't a real tunnel and the box
   * disconnects and reconnects as '_edge' once enrollment succeeds. */
  handleEnrollConnection(client) {
    this.attachInternalApiBridge(client);
  }
