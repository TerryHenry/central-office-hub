...
class TunnelServerManager extends EventEmitter {
    ...
    this.server = new Server(
      {
        ...
        // ssh2's own server-side default (15s x 3 = 45s) is tight for appliances that
        // may sit behind flaky links or just be settling in right after boot -- matched
        // here to the edge client's own keepaliveInterval/keepaliveCountMax
        // (lib/tunnelClient.js) so neither side gives up on the other first. Confirmed
        // in production: at the tighter default, a brief post-boot network hiccup was
        // enough to trip this and tear down an otherwise-fine connection.
        keepaliveInterval: 15000,
        keepaliveCountMax: 8
      },
      (client) => this.handleConnection(client)
    );
  ...
  handleConnection(client) {
    ...
    client.on('authentication', (ctx) => {
      ...
      if (admin && configStore.verifyPassword(ctx.password, admin.passwordSalt, admin.passwordHash)) {
        ...
        // (a shell on the hub) shouldn't bypass it.
  ...
  visibleChoices(caller, isAdmin) {
    ...
    if (isAdmin) {
      ...
      // Grant order here is chronological (whenever each grant happened to be added),
      // not grouped by site -- a port granted later (even to the same group as its own
      // site's other ports, e.g. because it was added to that site after initial
      // onboarding) would otherwise land wherever that grant call happened to fall,
      // disconnected from its own site's other entries. Sorted below instead of relied
      // on here.
    ...
    // Site name first, then port label within a site -- matches the appliance's own
    // Ports tab and the Sites table's own port ordering, so a caller's list actually
    // groups by site instead of reflecting whatever order grants happened to be added in.
    choices.sort(
      (a, b) =>
        a.site.name.localeCompare(b.site.name, undefined, { sensitivity: 'base' }) ||
        a.port.label.localeCompare(b.port.label, undefined, { sensitivity: 'base' })
    );
    return choices;
  }
