...
/** The local logins a given site should carry, for a fleet "sync-users" command. scope
 * 'access' (default): every
...
function usersForSiteSync(siteId, scope = 'access') {
  const out = [];
  for (const user of state.users) {
    ...
    // scope 'all' (an admin's explicit "every hub user" choice) also includes users with no
    // reach into this site; those get their own "access on pushed sites" level.
    const unrelated = grants.length === 0 && !pushed;
    if (unrelated && scope !== 'all') continue;
    const canWrite = grants.some((g) => g.permission === 'read-write') || ((pushed || unrelated) && normalizeEdgePermission(user.edgePermission) === 'read-write');
    out.push({
...
/** Remembers how a site's users are synced ('access' = only users with reach into it, 'all' =
 * every hub user) so later automatic re-syncs keep the same shape. */
function setSiteUserSyncScope(siteId, scope) {
  const site = findSiteById(siteId);
  if (!site) throw new Error('site not found');
  site.userSyncScope = scope === 'all' ? 'all' : 'access';
  persist();
}
