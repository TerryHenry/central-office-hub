...
const defaults = {
  ...
  alerts: {
    ...
    // On by default alongside the two above -- a site going on battery/low battery
    // usually precedes it going offline, so it's the same kind of "want to know right
    // away" event, not a routine one.
    notifyOnUpsPower: true,
  ...
  enrollmentTokens: [],
  // Saved batch-action scripts ({id, name, script}). Credentials belong in the run dialog
  // ({{username}}/{{password}}), not in a template.
  batchTemplates: [],
...
function mergeWithDefaults(raw) {
  return {
    ...
    batchTemplates: Array.isArray(raw.batchTemplates) ? raw.batchTemplates : [],
...
function deleteSite(id) {
  ...
  // Same reasoning as removePort's own cleanup above -- otherwise a group's grants keep
  // referencing a site that no longer exists, and grantsForUser() never cross-checks a
  // grant against the live site list, so it dangles forever (surfacing as raw UUIDs
  // where the Groups tab expects a site/port name, since the lookup it does to render
  // that name just fails silently).
  for (const group of state.groups) {
    group.grants = group.grants.filter((g) => g.siteId !== id);
  }
  persist();
}
...
function recordHeartbeat(siteId, { version, ports, sshEnabled, webTerminalEnabled, remoteShellEnabled, ups, adminsFingerprint, tftp, hubKeyPinned }) {
  const site = findSiteById(siteId);
  if (!site) throw new Error('site not found');
  site.lastSeenAt = new Date().toISOString();
  if (version) site.reportedVersion = version;
  // Entirely overwritten each heartbeat, same as ports above -- null (UPS monitoring
  // off, not configured, or the site's own live query just failed that cycle) replaces
  // whatever was there just as readily as a real reading does, so a site that stops
  // reporting UPS data doesn't leave a stale "on battery" reading stuck forever.
  site.ups = ups || null;
  ...
  if (typeof remoteShellEnabled === 'boolean') site.edgeRemoteShellEnabled = remoteShellEnabled;
...
/** Creates a template, or replaces the one with the same id (or the same name, so saving under an existing name updates it). */
function saveBatchTemplate({ id, name, script }) {
  const existing = state.batchTemplates.find((t) => (id && t.id === id) || t.name.toLowerCase() === name.toLowerCase());
  if (existing) {
    existing.name = name;
  ...
  const template = { id: crypto.randomUUID(), name, script, updatedAt: new Date().toISOString() };
  state.batchTemplates.push(template);
  persist();
  return template;
}

function deleteBatchTemplate(id) {
  state.batchTemplates = state.batchTemplates.filter((t) => t.id !== id);
  persist();
}
