...
const defaults = {
  ...
  batchTemplates: [],
  // Named devices on a site's LAN ({id, name, siteId, host, port, notes}) that an admin can reach
  // with an SSH port forward through that site's tunnel.
  forwards: [],
...
function deleteSite(id) {
  ...
  state.forwards = state.forwards.filter((f) => f.siteId !== id);
  persist();
}
...
// ---------- Forwards (a named device on a site's LAN an admin can reach through its tunnel) ----------
function listForwards() {
  return state.forwards;
}

function findForwardById(id) {
  return state.forwards.find((f) => f.id === id);
}

/** Case-insensitive: the name arrives as the "host" of an SSH port-forward request. */
function findForwardByName(name) {
  const wanted = String(name || '').toLowerCase();
  return state.forwards.find((f) => f.name.toLowerCase() === wanted);
}

/** Creates a forward, or updates the one with the same id. Names are unique. */
function saveForward({ id, name, siteId, host, port, notes }) {
  if (!findSiteById(siteId)) throw new Error('site not found');
  const clash = findForwardByName(name);
  if (clash && clash.id !== id) throw new Error(`there is already a forward named "${clash.name}"`);
  const existing = id ? findForwardById(id) : null;
  if (id && !existing) throw new Error('forward not found');
  const fields = { name, siteId, host, port, notes: notes || '' };
  if (existing) {
    Object.assign(existing, fields);
    persist();
    return existing;
  }
  const forward = { id: crypto.randomUUID(), ...fields, createdAt: new Date().toISOString() };
