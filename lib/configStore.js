...
function recordHeartbeat(siteId, { version, ports }) {
  const site = findSiteById(siteId);
  if (!site) throw new Error('site not found');
...
// ---------- Pending commands ----------
function queueCommand(siteId, command) {
  if (!findSiteById(siteId)) throw new Error('site not found');
  state.pendingCommands.push({ siteId, command, queuedAt: new Date().toISOString() });
  persist();
}
