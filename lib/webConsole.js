...
class WebConsoleManager extends EventEmitter {
  ...
  attach(httpsServer, sessionMiddleware) {
    ...
    httpsServer.on('upgrade', (req, socket, head) => {
      ...
      // sameSite: 'lax' on the session cookie does NOT protect a WebSocket handshake the
      // way it protects an ordinary POST -- the upgrade request is technically a GET, which
      // Lax lets ride along cross-site. A malicious page could otherwise open a WebSocket
      // to this endpoint using an admin's own cookies and drive a live tunneled console
      // session through it. Browsers always send Origin on a WebSocket handshake, so
      // requiring it to match this server's own host closes that the same way the CSRF
      // token closes it for HTTP requests.
